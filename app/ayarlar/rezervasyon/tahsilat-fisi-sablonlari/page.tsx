...
import { Plus, Edit, Trash2, FileText, X, CheckCircle2, CircleDot } from 'lucide-react';
...
export default function TahsilatFisiSablonlariPage() {
  ...
  const fetchTemplates = async () => {
    try {
      setLoading(true);
      const res = await fetch('/eventra/api/tahsilat-fisi-sablonlari', {
        cache: 'no-store',
      });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let res;
      if (editingTemplate) {
        res = await fetch('/eventra/api/tahsilat-fisi-sablonlari', {
          method: 'PUT',
  ...
  const handleDeleteConfirm = async () => {
    ...
    try {
      const res = await fetch(`/eventra/api/tahsilat-fisi-sablonlari?id=${deletingTemplateId}`, {
        method: 'DELETE',
      });
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-3xl font-bold text-gray-900 dark:text-gray-100">
            Tahsilat Fişi Şablonları
          </h2>
          <p className="text-gray-600 dark:text-gray-400 mt-1">
            Tahsilat fişleri için şablonları yönetin.
          </p>
        </div>
            ...
            <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto p-6 space-y-4">
              <div>
                ...
                <input
                  ...
                  placeholder="Örn: Standart Tahsilat Fişi"
                  required
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Fişin üst kısmında görünecek içerik (logo, firma bilgileri vb.)"
                  rows={4}
                />
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-none"
                  placeholder="Fiş içeriğini buraya yazın. Değişkenler için {{değişken_adı}} formatını kullanabilirsiniz."
                  rows={10}
                  required
                />
                <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
                  Örnek değişkenler: {'{{musteri_adi}}'}, {'{{tutar}}'}, {'{{tarih}}'}, {'{{odeme_yontemi}}'}
                </p>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full px-4 py-2 rounded-lg bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:ring-2 focus:ring-blue-500 focus:border-transparent outline-none resize-none"
                  placeholder="Fişin alt kısmında görünecek içerik (notlar, imza alanı vb.)"
                  rows={4}
                />
              </div>
              ...
              <div className="flex items-center gap-4">
                <label className="flex items-center gap-2 cursor-pointer">
                  ...
                  />
                  <span className="text-sm text-slate-700 dark:text-slate-300">Varsayılan Şablon</span>
                </label>
                <label className="flex items-center gap-2 cursor-pointer">
                  <input
                    type="checkbox"
