...
// Tüm rezervasyon kaynaklarını getir
export async function GET(request: NextRequest) {
  try {
    const sources = await prisma.rezervasyonKaynak.findMany({
      where: {
        isActive: true,
      },
      orderBy: {
    ...
    return NextResponse.json(
      { error: 'Rezervasyon kaynakları getirilemedi', message: error.message },
      { status: 500 }
    );
  }
}

// Yeni rezervasyon kaynağı oluştur
export async function POST(request: NextRequest) {
  try {
    ...
    return NextResponse.json({
      success: true,
      source,
      message: 'Rezervasyon kaynağı başarıyla oluşturuldu',
    });
  } catch (error: any) {
    console.error('Rezervasyon kaynağı POST error:', error);
    return NextResponse.json(
      { error: 'Rezervasyon kaynağı oluşturulamadı', message: error.message },
      { status: 500 }
    );
  }
}
